    ...
    if len(product_names) < count:
        ...
        while len(product_names) < count:
            base_product = random.choice(
                category_products.get(category_name, default_products)
            )
...
def generate_transaction_data(
    products, user, toko, count=10, days_range=30, include_unpaid=True
):
    ...
    for i in range(count):
        ...
        is_unpaid = (
            include_unpaid and random.random() < 0.15
        )  # 15% chance of unpaid transaction
        status = "Belum Lunas" if is_unpaid else "Lunas"
